...
校友線上登記 ＋ 大樂透風格的現場抽獎系統。視覺採用世新大學 70 週年「校友返校日」紅金主視覺。

零相依套件，只要有 Node.js 就能跑；資料存在本機單一 JSON 檔，整個資料夾複製走就能搬家。
適合校友回娘家、系友會、尾牙等需要「先報名、現場投影抽獎」的場合。

```
校友掃 QR Code → 填資料登記 → 活動現場投影抽獎畫面 → 按下開始抽獎 → 5 秒後出現中獎姓名
```
...
| `/draw` | **抽獎畫面**。投影用，彩球攪拌約 4 秒後掉出中獎球並顯示姓名 | 是 |
...
4. **抽獎**：按「開始抽獎」，彩球攪拌後掉出中獎球，姓名同時顯示在球上與下方大字。
...
| 分頁 | 內容 |
|---|---|
| 配色 | 底色、紅色光暈、卡片、主色、亮金、按鈕文字、文字、輸入框、緞帶三色等 16 項 |
| 抽獎機 | 外環三色階、球槽、5 種彩球顏色、中獎球顏色 |
| 字型與尺寸 | 內文／標題／英文三種字型、卡片與按鈕圓角、各標題字級、中獎姓名字級、抽獎機大小 |
| 登記頁文字 | 緞帶、英文副標、標題、說明、每個欄位名稱與提示字、按鈕、個資說明、完成與截止畫面（31 項） |
| 抽獎頁文字 | 緞帶、英文副標、標題、剩餘人數、轉盤中心字、按鈕、「恭喜抽中」等（12 項） |
| 進階 CSS | 自由撰寫 CSS，優先權最高 |

- 預覽可切換登記頁／登記完成／登記截止／抽獎頁，以及電腦／手機寬度；預覽中的送出與抽獎都是假的，不會寫入資料。
- 每個改過的欄位旁有 ↺ 可單獨還原，另有「還原成已儲存」與「全部恢復預設」。
- 預設為紅金配色；緞帶「校友返校日」、英文副標、左上角名稱都能在「登記頁文字」「抽獎頁文字」修改。
...
public/
  ...
  design.html          設計後台
  assets/
    theme.css          共用樣式與 CSS 變數預設值（紅金配色、緞帶、彩球）
    design.js          套用設計設定、預覽用的 postMessage 溝通
    logo-70.webp       70 週年 logo（登記頁主視覺，720px）
...
`draw.html` 裡有一段簡單的二維物理：球在圓形容器內受重力、隨機亂流與旋轉力，並處理球與外圈、球與中心軸、球與球的碰撞。按下抽獎後同時發送抽籤請求與播放攪拌動畫，攪拌 3.8 秒後停止，最接近出口的球掉出，接著顯示姓名，整體約 4.8 秒。
