...
```
校友掃 QR Code → 填資料登記 → 活動現場投影抽獎畫面 → 按下開始抽獎 → 約 5 秒先出現系所 → 再 3 秒揭曉姓名
```
...
| `/draw` | **抽獎畫面**。投影用，彩球攪拌後掉出中獎球，先顯示系所、3 秒後揭曉姓名大字 | 是 |
...
4. **抽獎**：按「開始抽獎」，彩球攪拌後掉出中獎球，先顯示「恭喜抽中」與**系所**（同名時靠系所區分，沒填系所會顯示「（未填系所）」），**3 秒後**姓名以大字揭曉，伴隨金色光芒、畫面閃光與兩波彩帶。
...
| 字型與尺寸 | 內文／標題兩種字型、卡片與按鈕圓角、各標題字級、中獎系所與姓名字級、抽獎機大小 |
